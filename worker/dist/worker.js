...
var ebayConnector = {
  meta: {
    ...
    enabled: false,
    ref: "apify:apify/cheerio-scraper",
    notes: "Disabled: eBay blocks the cheerio scraper. Re-enable only with a JS/browser actor (the keyword-search page-function/map below still apply)."
  },
  isConfigured(ctx) {
    return Boolean(ctx.env("APIFY_TOKEN"));
  },
      ...
      <table role="presentation" class="container" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:600px;background-color:#ffffff;border:1px solid #d9d6cf;">
        ...
        <tr>
          <td class="px" style="padding:8px 32px 0;" bgcolor="#ffffff">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:2px solid #0d0d0d;"><tbody>
              ...
              <tr><td style="font-family:'Courier New',Courier,monospace;font-size:11px;letter-spacing:2px;color:#0d0d0d;text-transform:uppercase;">Market Data \xB7 Right now</td></tr>
          ...
          <td class="px" style="padding:22px 32px 0;" bgcolor="#ffffff">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tbody><tr>
              <td class="stack" width="50%" valign="top" style="border:1px solid #e6e5e2;padding:18px 20px;">
                <div style="font-family:'Courier New',Courier,monospace;font-size:10px;letter-spacing:1px;color:#8a8a85;text-transform:uppercase;">Median asking \xB7 all air-cooled</div>
          ...
          <td style="padding:44px 0 0;" bgcolor="#ffffff">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" bgcolor="#0d0d0d" style="background-color:#0d0d0d;"><tbody><tr>
              <td class="px" style="padding:40px 32px;">
                <div style="font-family:'Courier New',Courier,monospace;font-size:11px;letter-spacing:2px;color:#cfcfca;text-transform:uppercase;">The Workshop \xB7 AI Service</div>
                ...
                <div style="font-family:'Courier New',Courier,monospace;font-size:11px;color:#8a8a85;padding-top:14px;">Free to browse \xB7 LUFT Pro $19/mo for the full guides &amp; unlimited AI</div>
              </td>
            </tr></tbody></table>
          </td>
        </tr>
        ...
        <tr>
          <td class="px" style="padding:44px 32px 0;" bgcolor="#ffffff">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:2px solid #0d0d0d;"><tbody>
              <tr><td style="height:18px;line-height:18px;font-size:0;">&nbsp;</td></tr>
              <tr><td style="font-family:'Courier New',Courier,monospace;font-size:11px;letter-spacing:2px;color:#0d0d0d;text-transform:uppercase;">Parts &amp; Spares \xB7 New</td></tr>
          ...
          <td class="px" style="padding:32px 32px 40px;" bgcolor="#ffffff">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:1px solid #e6e5e2;"><tbody>
              ...
              <tr><td style="font-family:'Courier New',Courier,monospace;font-size:11px;line-height:18px;color:#8a8a85;padding-top:12px;">
                Aggregated from 40+ US sources \xB7 Not affiliated with Dr. Ing. h.c. F. Porsche AG<br>
                You're receiving this because you signed up at driveluft.com.<br>
                <a href="${base}/unsubscribe" target="_blank" style="color:#8a8a85;text-decoration:underline;">Unsubscribe</a> \xB7 <a href="${base}" target="_blank" style="color:#8a8a85;text-decoration:underline;">driveluft.com</a>
              </td></tr>
            </tbody></table>
          </td>
        </tr>
...
var handler = {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/confirm" && request.method === "POST") {
      ...
      await ensureSubscribers(env.DB);
      const upd = await env.DB.prepare("UPDATE subscribers SET status = 'active' WHERE token = ? AND status = 'pending'").bind(b.token).run();
      const row = await env.DB.prepare("SELECT status, email FROM subscribers WHERE token = ?").bind(b.token).first();
