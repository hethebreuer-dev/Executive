...
function renderDigestHtml(cars: DigestRow[], env: Env): string {
  ...
  const photoOf = (c: DigestRow): string => {
    try {
  ...
  const metaOf = (c: DigestRow): string => {
    const loc = [c.city, c.state].filter(Boolean).join(", ");
    return [c.source, loc, c.mileage ? `${c.mileage.toLocaleString("en-US")} mi` : ""]
      .filter(Boolean)
      .join(" · ");
  };
  const site = env.APP_BASE_URL ? env.APP_BASE_URL.replace(/\/$/, "") : "";
  // Real source URL when we have one; otherwise the internal detail page keyed
  // by canonical id (seller-submitted rows carry no external url).
  const hrefOf = (c: DigestRow): string =>
    c.url && /^https?:\/\//i.test(c.url)
      ? c.url
      : site
        ? `${site}/listing/${encodeURIComponent(c.id)}`
        : "#";
  ...
  // The newest arrival headlines the email; the rest fill the "also new" list.
  const feat = cars[0];
  ...
  const featImg = featPhoto
    ? `<!--[if !mso]><!--><img src="${esc(featPhoto)}" alt="${esc(featTitle)}" width="536" style="width:100%;max-width:536px;height:auto;"><!--<![endif]--><!--[if mso]>[ ${esc(featTitle)} ]<![endif]-->`
    : esc(feat.model_family.toUpperCase());
  ...
  const rows = rest
    .map((c) => {
      ...
      const href = hrefOf(c);
      const rimg = photo
        ? `<a href="${esc(href)}" target="_blank"><img src="${esc(photo)}" alt="${esc(title)}" width="150" style="width:150px;height:auto;border:1px solid #e6e5e2;"></a>`
        : `<div style="width:150px;height:100px;background:#f1f0ed;border:1px solid #e6e5e2;"></div>`;
      return `
          <tr>
        ...
        <table role="presentation" class="container" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:600px;background-color:#ffffff;border:1px solid #d9d6cf;">
          ...
          <tbody><tr>
            <td class="px" style="padding:34px 32px 0;" bgcolor="#ffffff">
              ...
              <div style="font-family:'Courier New',Courier,monospace;font-size:11px;letter-spacing:3px;color:#8a8a85;text-transform:uppercase;padding-top:8px;">Air-Cooled · New Today</div>
            </td>
          </tr>
          ...
          <tr>
            <td class="px" style="padding:0 32px;" bgcolor="#ffffff">
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:2px solid #0d0d0d;">
                ...
                <tr>
                  <td style="font-family:'Courier New',Courier,monospace;font-size:11px;letter-spacing:2px;color:#0d0d0d;text-transform:uppercase;">${esc(countLabel)} · ${esc(dateLabel)}</td>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border:1px solid #e6e5e2;">
                ...
                <tr>
                  <td style="padding:22px 24px 24px;">
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                      <tbody><tr>
                        <td bgcolor="#0d0d0d" style="font-family:'Courier New',Courier,monospace;font-size:9px;letter-spacing:1px;color:#ffffff;text-transform:uppercase;padding:4px 8px;">Featured · New</td>
                      </tr>
                    </tbody></table>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:1px solid #e6e5e2;">
                ...
                <tr>
                  <td style="font-family:'Courier New',Courier,monospace;font-size:11px;line-height:18px;color:#8a8a85;padding-top:12px;">
                    Aggregated from 40+ US sources · Not affiliated with Dr. Ing. h.c. F. Porsche AG<br><br>
...
// plausible-priced active listings; live count is all active inventory; byGen
// is the median asking for each generation (only those with inventory), highest
// first.
...
async function marketSnapshot(
  env: Env
): Promise<{ median: number; liveCount: number; byGen: { label: string; median: number }[] }> {
  try {
    ...
    const rows = await env.DB
      .prepare("SELECT model_family AS mf, price FROM listings WHERE status = 'active' AND price >= 1000")
      .all<{ mf: string; price: number }>();
    ...
    const byGen = Object.keys(GEN_LABEL)
      .map((fam) => ({ label: GEN_LABEL[fam], median: medianOf(byFam[fam] || []) }))
      .filter((g) => g.median > 0)
...
function renderWelcomeHtml(
  env: Env,
  snap: { median: number; liveCount: number; byGen: { label: string; median: number }[] }
): string {
  ...
  // Median asking by generation — one row per generation with inventory.
  const genRows =
    snap.byGen.length > 0
      ? snap.byGen
          .map(
          ...
          .join("")
      : `<tr><td colspan="2" style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#8a8a85;padding:14px 0;border-bottom:1px solid #eeeeec;">Fresh numbers land with the next market update.</td></tr>`;
  return `<!DOCTYPE html>
<html lang="en"><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
